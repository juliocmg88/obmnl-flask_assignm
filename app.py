...
# Modify the get_transactions function to pass the total balance to the template
@app.route("/")
...
def delete_transaction(transaction_id):
    ...
    return redirect(url_for("get_transactions"))
# Search operation: Filter transactions within a specified amount range
@app.route("/search", methods=["GET", "POST"])
def search_transactions():
    if request.method == "POST":
        # Get the min and max amount values from the form
        min_amount = float(request.form["min_amount"])
        max_amount = float(request.form["max_amount"])

        # Filter transactions within the specified amount range
        filtered_transactions = [
            transaction for transaction in transactions
            if min_amount <= transaction["amount"] <= max_amount
        ]

        # Render transactions.html but with the filtered transactions
